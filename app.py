...
def build_pairs_from_classic(raw_bytes: bytes, filename: str) -> pd.DataFrame:
    ...
    out = pd.DataFrame({
        ...
        "total_cM": pd.to_numeric(df[tot_col], errors="coerce"),
...
with col_a:
    st.dataframe(df[["sample1", "sample2", "total_cM", "relationship_class"]].sort_values("total_cM", ascending=False).head(1000), width="stretch", height=240)
with col_b:
    min_cm_build = st.number_input("Minimum cM to build clusters", min_value=float(df["total_cM"].min()), max_value=float(df["total_cM"].max()), value=max(40.0, float(df["total_cM"].min())), step=1.0)
...
with right:
    ...
    if selected_pairs.empty:
        ...
        with fc1:
            min_cm_view = st.slider("Min total IBD (cM) in selected cluster", min_value=min_c, max_value=max_c, value=min_c, step=1.0)
        with fc2:
            min_cm_exact = st.number_input("Exact cM", min_value=min_c, max_value=max_c, value=min_cm_view, step=1.0)
